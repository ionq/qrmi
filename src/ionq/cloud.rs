...
use crate::ionq::open_qasm_to_qis::translate_qasm3_to_ionq_qis;
...
impl IonQCloud {
    ...
    fn resolve_job_type_and_input(&self, program: &str) -> Result<(&'static str, Value)> {
        ...
        if s.starts_with('{') {
            if let Ok(qis) = serde_json::from_str::<Value>(s) {
                if let Some(input) = qis.get("input") {
                ...
                if qis.get("qubits").is_some() && qis.get("circuits").is_some() {
                    return Ok((JOB_TYPE_MULTI_CIRCUIT, qis));
                }
                if qis.get("qubits").is_some() && qis.get("circuit").is_some() {
        ...
        if first_line_uc.starts_with("OPENQASM 3") {
...
impl QuantumResource for IonQCloud {
    ...
    async fn task_start(&mut self, payload: Payload) -> Result<String> {
        ...
        let (job_type, job_input) = self.resolve_job_type_and_input(&input)?;
...
";
    ...
    fn qasm3_on_qpu_translates_to_circuit() {
        ...
        assert!(input.get("qubits").is_some(), "translated QIS must have 'qubits'");
        assert!(input.get("circuit").is_some(), "translated QIS must have 'circuit'");
    ...
    fn qir_input_defaults_to_qir_type() {
        ...
        let (job_type, input) = cloud.resolve_job_type_and_input(qir).unwrap();
        assert_eq!(job_type, JOB_TYPE_QIR);
        assert_eq!(input["data"].as_str().unwrap(), qir);
    }

    #[test]
    fn circuit_json_returns_circuit_type() {
        let cloud = make_cloud("simulator");
        let circuit_json = r#"{"qubits": 2, "circuit": [{"gate": "h", "target": 0}]}"#;
        let (job_type, input) = cloud.resolve_job_type_and_input(circuit_json).unwrap();
        assert_eq!(job_type, JOB_TYPE_CIRCUIT);
    ...
    fn multi_circuit_json_returns_multi_circuit_type() {
        let cloud = make_cloud("simulator");
        let mc_json = r#"{"qubits": 2, "circuits": [{"circuit": [{"gate": "h", "target": 0}]}]}"#;
        let (job_type, _input) = cloud.resolve_job_type_and_input(mc_json).unwrap();
        assert_eq!(job_type, JOB_TYPE_MULTI_CIRCUIT);
    }

    #[test]
    fn wrapped_circuit_json_extracts_input() {
        let cloud = make_cloud("simulator");
        let wrapped = r#"{"input": {"qubits": 1, "circuit": [{"gate": "x", "target": 0}]}}"#;
        let (job_type, input) = cloud.resolve_job_type_and_input(wrapped).unwrap();
        assert_eq!(job_type, JOB_TYPE_CIRCUIT);
        assert_eq!(input["qubits"], 1);
    }

    #[test]
    fn qasm3_translation_produces_valid_gate_output() {
        let cloud = make_cloud("forte-1");
        let qasm = "\
OPENQASM 3.0;
qubit[1] q;
rx(3.14159265358979) q[0];
";
        let (job_type, input) = cloud.resolve_job_type_and_input(qasm).unwrap();
        assert_eq!(job_type, JOB_TYPE_CIRCUIT);
        let circuit = input["circuit"].as_array().unwrap();
        assert_eq!(circuit[0]["gate"], "rx");
        let rotation = circuit[0]["rotation"].as_f64().unwrap();
        assert!((rotation - 0.5).abs() < 1e-6, "pi radians should be 0.5 turns, got {rotation}");
    }
}
